...
class AccountMove(models.Model):
    ...
    def _l10n_ar_get_invoice_totals_for_report(self):
        ...
        if include_vat:
            tax_ids_filter = (lambda aml, tax: tax.id not in vat_taxes + nat_internal_other_taxes)
            tax_line_id_filter = (lambda aml, tax: tax.id not in vat_taxes + nat_internal_other_taxes)
        ...
        if self.l10n_latam_document_type_id.code in ['6', '7', '8']:
            ...
            # Compute original tax with all data
            ...
            for item in original_tax_total[0]['groups_by_subtotal'][_('Untaxed Amount')]:
